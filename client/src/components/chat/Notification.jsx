            ...
            {isOpen ? (
                <div className="notifications-box">
                    <div className="notifications-header">
                        <h3>Thông báo</h3>
                        <div className="mark-as-read" onClick={() => markAllNotificationAsRead(notification)}>đã xem tất cả</div>
                    </div>
                    {modifiedNotifications?.length === 0 ? <span>Không có thông báo...</span> : null}
                    {modifiedNotifications && modifiedNotifications.map((n, index) => {
                        return <div key={index} className={n.isRead ? 'notification' : "notification not-read"}
                        onClick={() => markNotificationAsRead(n,userChats,user,notification,setIsOpen(false))}>
                            <span>{`${n.senderName} gửi tin nhắn cho bạn`}</span>
