...
const registerUser = async (req, res) => {
  try {
    ...
    if (!name || !email || !password)
      return res.status(400).json("Không được bỏ trống các ô");
    if (!validator.isEmail(email))
      return res.status(400).json("Email phải hợp lệ.");
    if (!validator.isStrongPassword(password))
      return res.status(400).json("Hãy đặt một mật khẩu manh hơn");
